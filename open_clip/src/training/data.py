...
from .video_utils import RawVideoExtractor
...
class VideoCsvDataset(Dataset):
    ...
    def _get_text(self, video_id, caption=None):
        ...
        return words, choice_video_ids

    def _get_single_text(self, video_id):
        rind = random.randint(0, len(self.sentences[video_id]) - 1)
        caption = self.sentences[video_id][rind] #random sentcne 뱉음.
        words = self.tokenizer(caption)[0]
        return words

    def _get_rawvideo(self, choice_video_ids):
        video_mask = np.zeros((len(choice_video_ids), self.max_frames+1), dtype=np.longlong)
        ...
        for i, v_length in enumerate(max_video_length):
            video_mask[i][:v_length+1] = [1] * (v_length +1)

        return video, video_mask

    def __getitem__(self, idx):
        video_id, caption = self.csv['video_id'].values[idx], None
        words, choice_video_ids = self._get_text(video_id, caption)
        video, video_mask = self._get_rawvideo(choice_video_ids)
        return video, video_mask, words#, video_mask


def get_video_csv_dataset(args, preprocess_fn, is_train, epoch=0, tokenizer=None):
    input_filename = args.train_data if is_train else args.val_data
    assert input_filename

    dataset = VideoCsvDataset(
            csv_path= input_filename,
            json_path = args.json_caption_key,
